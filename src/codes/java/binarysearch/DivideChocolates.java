...
//Split the array into k non-empty continuous subarrays.
// Write an algorithm to maximize the minimum sum among these k + 1 subarrays.
public class DivideChocolates {
    int divideChocolates(int[] chocolates, int k)
    {
        ...
        while(low <= high)
        {
            ...
            if(isPossilbe(chocolates, k, mid))
            {
                ans = mid;
                low = mid + 1;
            }
    ...
    boolean isPossilbe(int[] chocolates, int k, int minSum)
    {
        int noOfFriends = 0;
        int currSum = 0;
